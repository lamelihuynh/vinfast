...
class Comment
{
    public static function create(int $userId, ?int $productId, string $body, int $rating = 5): bool 
    {
        global $pdo;
        $stmt = $pdo->prepare(
            'INSERT INTO comments (user_id, product_id, body, rating, is_approved)
             VALUES (:user_id, :product_id, :body, :rating, 0)'
        );
        return $stmt->execute([
            ':user_id'    => $userId,
