...
$avatarUrl = !empty($userData['avatar']) ? BASE_URL . ltrim((string)$userData['avatar'], '/') : $avatarFallback;
?>
<div class="rounded-3xl border border-slate-100 bg-white p-5 shadow-sm">
    <div class="flex items-center gap-4">
        <img src="<?= htmlspecialchars($avatarUrl) ?>" alt="avatar" class="h-16 w-16 rounded-2xl object-cover ring-2 ring-slate-100">
        <div class="min-w-0">
            <h3 class="truncate text-base font-bold text-slate-900"><?= $name ?></h3>
            <p class="truncate text-sm text-slate-500"><?= $email ?></p>
            <div class="mt-2 inline-flex rounded-full bg-blue-50 px-3 py-1 text-[11px] font-semibold uppercase tracking-[0.18em] text-blue-700">Thành viên</div>
        </div>
    </div>
